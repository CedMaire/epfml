...
def suppress_single_roads(labels, size_width, size_height):
    ...
    for i in range(1, size_width - 1):
        for j in range(1, size_height - 1):
            if labels[get_index(i, j, size_width)][0] <= THRESHOLD: #road
                if labels[get_index(i + 1, j, size_width)][0] > THRESHOLD and labels[get_index(i - 1, j, size_width)][0] > THRESHOLD and labels[get_index(i, j + 1, size_width)][0] > THRESHOLD and labels[get_index(i, j - 1, size_width)][0] > THRESHOLD: #surrounded by bgrd
...
def add_road_when_line_of_roads(labels, size_width, size_height, number_of_road_needed):
    ...
    for i in range(0, size_width):
        for j in range(number_of_road_needed, size_height - number_of_road_needed):
            if labels[get_index(i, j, size_width)][0] > THRESHOLD: #bgrd
                surroundedByRoad = True
                for idx in range(1, number_of_road_needed + 1):
                    if labels[get_index(i , j - idx, size_width)][0] > THRESHOLD:
    ...
    for j in range(0, size_height):
        for i in range(number_of_road_needed, size_width - number_of_road_needed):
            if labels[get_index(i, j, size_width)][0] > THRESHOLD: #bgrd
                surroundedByRoad = True
                for idx in range(1, number_of_road_needed + 1):
                    if labels[get_index(i - idx, j , size_width)][0] > THRESHOLD:
                        surroundedByRoad = False
                        break
                    if labels[get_index(i+ idx, j , size_width)][0] > THRESHOLD:
                        surroundedByRoad = False
...
def suppress_group_roads_surrounded(labels, size_width, size_height, size_square):
    ...
    for i in range(1, size_width - size_square):
        for j in range(1, size_height - size_square):
            ...
            one_patch_is_road = labels[get_index(i, j, size_width)][0] <= THRESHOLD 
            for k1 in range(1, size_square):
                for k2 in range(1, size_square):
                    one_patch_is_road = one_patch_is_road or labels[get_index(i + k1, j + k2, size_width)][0] <= THRESHOLD 
            if one_patch_is_road:
                #We verify that the outside part is indeed background
                outside_part = True
                for k in range(-1, size_square + 1):
                    outside_part = outside_part and labels[get_index(i - 1, j + k, size_width)][0] > THRESHOLD 
                    outside_part = outside_part and labels[get_index(i + size_square, j + k, size_width)][0] > THRESHOLD 
                    outside_part = outside_part and labels[get_index(i + k, j - 1, size_width)][0] > THRESHOLD 
                    outside_part = outside_part and labels[get_index(i + k, j + size_square, size_width)][0] > THRESHOLD 
                if outside_part:
